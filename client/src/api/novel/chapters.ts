...
} from "@0xnovelagent/shared/types/timeline";
import type {
...
export async function getChapterTraces(novelId: string, chapterId: string) {
  const { data } = await apiClient.get<ApiResponse<import("@0xnovelagent/shared/types/agent").AgentRun[]>>(
...
export async function generateChapterExecutionContract(
  ...
  chapterId: string,
  payload: Partial<{
    provider: import("@0xnovelagent/shared/types/llm").LLMProvider;
    model: string;
    temperature: number;
  }> = {},
) {
