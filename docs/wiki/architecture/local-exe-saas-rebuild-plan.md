...
生成开始后立即展示三个固定候选位置，不使用占满内容区的单一转圈等待框。尚未返回的字段使用与最终内容结构一致的骨架，模型片段到达后逐步替换为标题、一句话亮点、主角、核心冲突和阅读感觉，让用户在等待期间理解结果会是什么。

三个候选仍属于同一次付费结构化调用，不得为了视觉上的“同时生成”拆成三笔模型请求。流式片段只用于本地预览，按时间或字符数批量写入当前 `ConsumerCreationOperation`；完整结果通过 Schema 校验并进入待确认状态后，用户才能选择方向。预览解析或预览落盘失败不能触发模型重试，也不能阻止最终完整结果保存。

每个方向的暂定书名在同一次调用内部先比较多个候选，只输出最终一个。书名必须同时让用户感知题材和本故事独有的冲突、能力、规则、身份差或关键意象，禁止使用可放到任意作品上的抽象万能标题；三个方向的书名不得只替换一个名词。

每个方向还必须冻结四项可被后续规划持续消费的商业连载信息：

- 核心优势：技能、资源、身份、关系、知识或特殊能力。
- 优势限制：条件、代价、盲区或失败风险。
- 持续回报：读者可以反复获得、但表现形式必须变化的兑现方式。
- 成长路线：能力、地位、财富、关系、认知或行动范围的阶段变化。

这四项不等同于强制系统流、超能力、打脸或升级换地图，必须服从用户题材。
...
卷名必须取自本卷不可互换的地点、物件、关系变化、制度冲突或关键转折；相邻卷不能重复同一核心词、尾词或句式。候选比较仍在既有卷规划调用内部完成，不增加费用。
...
第一段规划必须落实题材自适应的黄金前三章：第一章出现具体压力并让主角行动，最迟第二章展示核心优势及其限制，最迟第三章兑现首次回报或产生由主角行动引发的不可逆新问题。不强制生死危机、系统激活或打脸。
...
默认逐章确认继续保留，同时提供一次授权写完当前剧情阶段的可选模式。它不是一笔跨多章的中转请求，而是本地按顺序调度现有逐章操作：

1. 用户确认本段章节范围和计费说明。
2. 系统一次只生成一章，并持续流式保存。
3. 每章成功后自动保存正式版本，再开始下一章。
4. 最后一章保存后停止，进入下一次阶段或卷末检查。
...
“写完这一段”的确认区必须展示章节范围、剩余章数、当前余额和可取得的历史费用参考；没有真实历史样本时只说明按实际用量计费，不能虚构总价。运行中展示当前章、总进度、本段累计积分、本段累计 Token，并提供“写完当前章后暂停”。

连续创作的起点和剩余章数必须由作品最新章节决定，不能使用用户当前正在浏览的章节。用户回看历史章节时，只展示“正在查看历史章节”和“回到最新章节继续”，不得出现会从旧章节重新开始的“下一章”或“写完这一段”入口。

连续创作必须遵守：

- 不能跨过当前剧情阶段或卷的边界。
- 每章仍是独立 `ConsumerCreationOperation`，独立保存、计费和恢复。
- 网络中断、余额不足、生成失败或结果未知时立即暂停，不能自动重放付费请求。
- 暂停在当前模型调用结束后生效，避免主动制造半章。
- 软件退出或重启后保持暂停，由用户明确恢复。
- 用户可以返回阅读已完成章节；恢复连续创作时从本段第一章未完成或下一章继续。
- 不提供跨阶段、跨卷或整本书的后台无人值守持续扣费。
...
检查或跳过完成时，模型已经生成并保存下一段详细推进计划。界面不能直接退回含糊的“继续下一章”，必须先显示下一段名称、章节范围和目标，让用户知道后续创作已有规划，再选择逐章写或写完这一段。
...
| 路由职责 | 默认模型 | Prompt |
|---|---|---|
| 规划分析 | `deepseek-v4-flash` | 全部卷规划、当前剧情阶段、下一章任务、后续剧情调整、阶段过渡 |
| 正文创作 | `qwen3.7-plus` | 故事方向、第一章、章节生成、续写、AI 修改、整章重写 |
| 关键把关 | `claude-sonnet-4-6` | 全书骨架、阶段/卷末审校 |

Prompt ID 是路由的稳定依据，任务类型只负责未登记 Prompt 的兜底分类。发布环境可以用 `OXNOVEL_RELAY_PLANNER_MODEL`、`OXNOVEL_RELAY_WRITER_MODEL` 和 `OXNOVEL_RELAY_REVIEW_MODEL` 分别覆盖三层；`OXNOVEL_RELAY_MODEL` 只作为需要临时统一模型时的全局运维覆盖。

中转尚未提供 AI 请求幂等、状态查询和结果取回合同。在该合同补齐前，网络失败、协议失败或结果未知都不得自动切换模型重放付费请求；用户手动重试必须创建新的付费操作。
...
- 产品统一显示“0x积分”。
- `1 元人民币 = 1 0x积分`。
- 不向普通用户展示 Quota、模型倍率、上游币种或逐次请求技术明细；章节完成区可以展示易懂的本章输入、输出和总 Token 汇总，用于解释实际用量。
...
- 本章预计消费范围。
- 本章实际累计消费。
- 本章输入、输出、总 Token 与模型调用次数。

实际积分只采用中转返回的余额和累计消费差额。中转尚未完成入账或差额仍为零时显示“实际积分正在与中转同步”，不得把 `0.000 0x积分` 展示为已确认实际消费；Token 由本地逐次收集中转模型响应的 usage，并按章节内全部操作累计。
...
当前进度：共享契约、中转适配层、强制登录页、同步注册/登录、专用 Token 查找与复用、Token 内存边界、Windows `safeStorage`、匿名 `profileId` 与独立 SQLite、余额、消费记录、微信 Native 下单和订单恢复已经实现。注册/登录字段与 `sk-` 规范已经冻结并纳入发布门禁；只剩真实小额支付和真实模型扣费对账需要生产凭证验收。

范围：

- 冻结同步注册和登录接口。
- 软件完成“注册或登录 → 查找/创建专用 Key → 取得并规范化 `sk-` Token → 校验余额”的同步链路；不要求中转注册接口直接返回 Token。
...
- 故事方向使用同一次结构化调用的流式片段生成三个只读预览；三个候选位置从请求开始即显示，标题、亮点、主角、冲突和阅读感觉随本地轮询逐步出现，不增加模型调用次数。
...
- SQLite 与 PostgreSQL Prisma Schema 均通过 `prisma validate`。
- 状态机测试覆盖五次生成/五次确认、重复请求幂等、失败后新请求恢复、进程中断结果未知、单次调用的故事方向流式预览和第一章候选稿采用边界。
...
- 连续创作只能复用已通过聚焦回归的逐章操作；正式发布前仍需在全新 Windows 环境完成前三章端到端验收。
...
- 下一章任务、正文生成和中断续写分别使用 `consumer.chapter.task@v4`、`consumer.chapter.write@v3` 和 `consumer.chapter.continue@v3`，全部经过 Prompt Registry 与自有中转模型路由。
- 第一章和后续章节共用章节标题策略：标题取自本章实际发生的具体动作、物件、地点、冲突、决定或短对白，不写“第 N 章”、不剧透章末答案，并避开最近章节的核心词和句式。模型偶尔不遵守格式时，服务端只做本地标题符号清洗，不追加模型调用。
- 第一版正文长度合同固定为目标 2600 个中文非空白字符、可接受范围 2200～3200 字；服务端会把模型返回的章节任务统一到产品目标，并收紧正文输出 Token 上限。生成后记录实际字数，只以本地规则提示长度越界、套话密集或总结式收尾，不自动重写、不增加隐藏模型调用。
- 首章、逐章正文、断点续写、AI 修改与整章重写复用同一正文策略：开头前 150 字必须承接人物和具体事件，有限视角不擅自解释他人内心，对话避免设定讲解，句式长短自然变化，章末停在具体动作、发现、决定、后果或未回答互动，禁止总结主题和强行升华。
- 流式正文按字符数或时间批量写入 `ConsumerChapterDraft`，并使用 revision 乐观锁；目标章节的正式正文在用户确认前保持不变。
- 创作台以约 350ms 的本地操作轮询把已接收正文投影到编辑器，流式草稿按约 80 字或 200ms 刷盘；只读编辑器保持文本光标，不显示会被误解为卡死的等待光标。
...
- 工作台右侧只展示当前下一步、生成状态、历史同类消费参考、本章实际积分、输入/输出/总 Token 和只读创作进度，不暴露任务、模型、导演或审校状态机。
- 桌面端保留左侧章节目录；目录始终以本地章节顺序显示“第 N 章”，AI 标题只能作为副标题，避免标题替换后被误认为章节丢失；每章同时显示当前草稿的非空白字符数，并在手工编辑或流式生成时更新。窄屏端正文优先，目录与历史版本改为可访问对话框，下一步区域排在正文之后，不再挤压编辑器。
- `ConsumerCreationOperation` 持久化输入、输出、总 Token 和调用次数；单章汇总包含任务规划、正文以及中断后续写等该章全部操作。实际积分通过生成前后的中转可用余额与累计消费双检查点确认，未取得正向差额时保留为待同步而不是写入零值。

已实现的连续创作链路：

- 阶段检查或跳过完成后，不再直接回到通用“继续下一章”，而是先展示已保存的下一段名称、章节范围、目标和主要推进方向。
- 同时提供“只写下一章”和“写完这一段”两种选择；连续模式以本地持久化批次顺序调用现有逐章操作，每章独立保存和计费，只运行到当前阶段边界。
- 连续模式提供当前章/总章数、本段积分与 Token 汇总及“写完当前章后暂停”；异常、离线、余额不足、结果未知、退出或重启均自动暂停，禁止自动重试收费请求。
- `ConsumerSegmentRun` 持久化阶段边界、当前章节、子操作列表和暂停状态；客户端恢复作品时读取最新批次，运行中轮询进度并自动打开正在生成的章节。
...
- 连续创作聚焦测试覆盖阶段内顺序生成与用量汇总、章末暂停和恢复、子操作记录前中断的目标章复用、最终本地版本提交失败时不重复调用模型，以及程序重启后把旧运行状态恢复为暂停并从按钮建立新的执行时间点继续。
...
- `consumer.chapter.revise@v4` 与 `consumer.chapter.rewrite@v4` 通过自有中转模型出口并复用逐章正文合同。修改只处理用户点名的问题；整章重写可重组本章场景、行动、对话和节奏，但不得重规划故事、改变已确认事实或强制套用打脸、奖励、断章等固定套路。
- 修改/重写只投影全书核心承诺、当前卷、当前阶段和最近章节末尾 1600 字，不再把全部远期卷规划与结局发送给模型；输出会清理完整外围代码围栏，不足 500 个非空白字符时按疑似中断拒绝创建候选稿。
...
- 阶段末章节生成完成后，创作台必须立即重新读取检查点，并用上述两个入口替换普通“继续下一章”区；若用户在短暂状态竞争中点击了旧按钮，收到服务端检查点阻断后也必须自动切换到处理区并清除错误提示，不能让用户自行寻找入口。
- 阶段/卷末检查通过 `consumer.story.review@v2` 生成结构化结果。上下文只保留阶段内所有锁定章节的开头/结尾证据与最近两章高清尾段，并明确“片段未出现不等于正文缺失”；远期卷内容不会进入局部审校。结果只展示整体结论、可定位影响位置、保持不变的内容和一个推荐动作，不创建普通用户需要调度的质量任务。
- 主动“调整后续剧情”通过 `consumer.story.adjust@v2` 判断低/中/高影响范围；低影响只能更新覆盖下一章的当前阶段，中影响最多更新完整卷规划，只有用户改变核心前提、全书阅读承诺或结局时才允许高影响更新全书骨架。该级联边界同时由 Prompt 和输出校验约束。
- 跳过检查后的 `consumer.story.transition@v2` 不做质量判断，只读取章节索引、最近两章高清尾段、全书稳定约束与下一章相邻卷窗口；每个阶段 beat 必须写清人物行动、具体阻力和可观察结果，禁止“冲突升级”“危机逼近”等抽象占位语。
...
- [x] 冻结注册请求字段。
- [x] 冻结登录请求字段。
- [x] 确认注册与登录链路最终取得完整可用 `sk-` Token。
- [x] 冻结同一用户 Token 的查找、复用、轮换和失效规则。
- [x] 在契约测试中验证同一 Token 对 AI、余额、日志和充值接口均有效。
- [x] 验证 AI 普通文本、流式正文和结构化输出。
- [ ] 用真实小额订单验证支付金额与积分到账 1:1；代码已强制拦截 `money !== amount`，等待真实已支付订单号留证。
- [ ] 用真实模型调用验证余额减少与消费日志一致；实扣验收脚本已经完成，等待真实 Token 显式开启。
...
| AI 输出 | OpenAI 兼容 `/chat/completions` 同时覆盖普通响应、SSE 流式响应和结构化响应；产品按 Prompt 隐藏路由到 `deepseek-v4-flash`、`qwen3.7-plus` 或 `claude-sonnet-4-6`，用户不需要选择 | 协议解析失败按本次操作失败/结果未知处理，不自动切换模型重放付费请求 |
...
```powershell
pnpm run verify:relay-contract
```

该命令会构建共享契约和服务端，并执行认证、Key 生命周期、余额/日志/充值、普通/流式/结构化 AI 的契约测试；桌面正式暂存流程也会自动执行它。

真实中转验收只在人工提供生产凭证时运行，不会被普通构建误触发：
...
$env:OXNOVEL_RELAY_PLANNER_MODEL = "deepseek-v4-flash"
$env:OXNOVEL_RELAY_WRITER_MODEL = "qwen3.7-plus"
$env:OXNOVEL_RELAY_REVIEW_MODEL = "claude-sonnet-4-6"

# 已支付一笔真实小额订单后再填写；脚本只查询，不创建或支付订单
$env:OXNOVEL_RELAY_CONTRACT_PAID_ORDER_NO = "真实订单号"

# 明确接受规划、正文和审校模型各一次最小真实扣费后才开启；
# 如只需验证单个模型，可改用 OXNOVEL_RELAY_MODEL 做全局覆盖
$env:OXNOVEL_RELAY_CONTRACT_RUN_MODEL = "true"
pnpm run verify:relay-contract:live
```

真实验收通过的判定必须同时满足：

1. 同一 `sk-` Token 可读取余额、消费日志和支付信息。
2. 已支付订单状态为 `paid`，且 `money === amount`。
3. 规划、正文和审校三个模型的最小调用均返回正文；每次调用后 `balance` 下降、`usedBalance` 上升且出现消费日志。
4. 命令输出不得记录 Token；验收记录只保存订单号、调用 ID、前后余额和日志 ID。
...
- [x] 默认逐章确认，并可选写完当前剧情阶段；每章独立计费、保存和恢复。
- [ ] 状态不确定的请求不自动重试。
- [ ] 流式中断保留已接收正文。
- [ ] 充值订单关闭页面后仍可恢复。
...
1. 注册、登录、Key 查找/创建/取明文和 `sk-` 规范已经按当前 Apifox 接口冻结；但上游注册/登录页面仍使用 `GenericObject`，中转变更字段时必须先更新契约测试再发布。
2. 同一 Token 的 AI、余额、日志和充值协议已经通过本地契约门禁；仍需用生产 Token 跑一次真实验收并保存脱敏证据。
3. `1 元 = 1 0x积分` 的运行时拦截和验收脚本已实现，仍需一笔真实已支付小额订单和一次最小模型调用完成最终对账。
4. AI 请求幂等、状态查询、结果取回和消费上限目前没有中转契约。
5. 商业授权或经律师确认的开源履约路线尚未完成。
6. 本机数据删除已实现；中转账号注销、退款、人工支持、法务定稿、真实代码签名和正式更新源仍未完成。
7. 中转没有任务级计价预估或可冻结的模型费率接口；当前只能用本机同类历史操作给出参考区间，首次调用无法提供可靠数值预估。
8. 发布流水线已生成 SBOM 和第三方许可证清单，但许可证信息不完整的依赖仍需人工审查。
9. 尚未在全新 Windows 机器使用真实中转完成安装到前三章、重启、断网、支付中断和换账号的完整验收。
