...
- 用户界面只展示产品化能力和必要的模型档位，不暴露供应商内部名称或技术参数。
...
- 金额展示跟随 `/api/usage/payment/info` 返回的 `quotaDisplayType`，不得硬编码人民币符号；Token 和内部 Quota 默认不直接展示给普通用户。

当前中转契约无法严格执行多章连续创作的最高消费上限，因此第一版只开放逐章确认，不开放“连续写到剧情阶段结束”。未来只有在中转提供可靠的请求幂等、请求级扣费关联和可执行消费上限后，才重新评估连续创作。
...
- [ ] 确认 `quotaDisplayType`、充值金额和账户余额的最终用户展示关系。
...
- [ ] 金额符号和单位跟随中转 `quotaDisplayType`，不硬编码人民币。
...
6. `quotaDisplayType`、微信充值金额、账户余额和内部 Quota 的最终展示关系需要统一。
