...
1. 可用的全书故事宏观骨架。
2. 覆盖全部卷的战略和卷骨架。
3. 当前 beat 的详细推进计划和可执行章节列表。
4. 第一章的 JIT 执行合同。

全部卷规划必须覆盖每卷职责、开始局面、核心目标、主要冲突、阶段成长、关键角色与势力、必须兑现项、重大转折、卷末高潮、下一卷牵引和预计章节范围。

这里的“全部卷相对完整”不表示提前拆出全部章节。节奏板仍只需要详细展开当前创作窗口，章节列表仍按当前 beat 增量生成，执行合同仍按下一章 JIT 补齐。

未来 soft 卷可以保留事件顺序、具体角色调度和章节数量的弹性，但不能缺少卷级职责、主要冲突、阶段兑现、卷末高潮和后续牵引。soft 表示可调整，不表示未规划。

普通用户不逐项审批四层资产。创建页只展示“搭建完整故事、安排各卷发展、准备开篇剧情、创作第一章”等进度；完成后默认进入第一章，规划资产在作品目录中可查看和编辑。

变更使用最小影响范围：

- 局部正文变化默认只更新下一章任务和必要连续性状态。
- 当前 beat 偏离只重算未写的当前 beat 范围。
- 当前卷目标变化只重算当前卷和未来卷承接检查。
- 只有全书核心命题、最终目标或主要矛盾变化时，才允许重建全书骨架和未来卷规划。
- 已有正文和已完成卷始终锁定，除非用户在明确影响提示后主动确认修改。
...
C 端不把上述内部状态直接交给普通用户。统一入口为“调整后续剧情”，由系统把变化映射到最小的未来未写范围：

- 文字表达变化不触发规划更新。
- 小型事实变化只刷新事实状态与下一章任务。
- 章级事件变化刷新下一章任务和当前剧情阶段。
- 阶段目标变化只调整当前阶段剩余内容。
- 卷目标变化调整当前卷剩余内容，并检查未来卷承接。
- 核心角色、主线、故事前提或结局变化，才进入当前卷、未来卷或未来全书骨架。

已完成正文和已完成卷保持锁定。每次应用调整前必须保存规划版本，之后作废未执行的旧任务并生成新的下一章任务；连续创作只允许低影响变化自动吸收，中高影响变化必须在章节边界暂停，高影响变化还必须在下一次付费调用前确认。

面向用户的调整摘要只回答变化、影响、不变内容和推荐接法。`affectedBeats`、`staleBeatCount`、`lockedBeatCount`、重跑节奏板或重跑卷战略等技术信息保留在内部决策层或高级诊断入口。
