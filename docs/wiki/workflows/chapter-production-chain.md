...
```text
轻量预检 -> 整章正文生成 -> 确定性基础校验 -> 保存稳定版本
                                            |
                                            v
                              必要状态闭合 / 异步资产回灌

按执行计划可选：AI 接收闸门 -> 局部修文 -> 复审
```

正文热路径只负责尽快生成、完成确定性基础校验并保存稳定章节。启用逐章 `autoReview` 或专业质量计划时，才追加 AI 接收闸门、局部修复和复审。章节稳定后由 `artifact_delta` 通道一次性回灌摘要、硬事实、状态快照、角色资源、关系动态、信息边界和伏笔 delta；全量伏笔校准保留为条件性后台对账。

## C 端默认连续创作计划

底层章节运行时保留接收门、局部修复、深度审校和复审能力，但 C 端默认执行计划不再要求每章都运行完整 AI 审校闭环。
...
-> 下一章必要事实与连续性状态闭合
-> 后台资产增量同步
-> 用户确认下一章，或连续运行到剧情阶段边界
```

C 端正文统一执行同一份写作合同：

- 首章、逐章正文、断点续写、AI 修改和整章重写共用目标 2600 字、可接受范围 2200～3200 字的长度策略。
- 下一章任务中的 `suggestedWords` 由服务端统一为产品目标，不能由模型自由扩大；正文输出 Token 上限只用于防止明显失控，不按长度截断可读正文。
- 开头必须尽快落到人物、动作或具体麻烦；章节任务明确拆分 `openingBeat`、可见的 `endingBeat`、只能留给读者判断的 `readerInference` 和禁止旁白直说的 `forbiddenExplanation`，旧任务在读取时由已有承接、钩子和结束状态兼容补齐。
- writer 在内部复查最后 300 字：有效动作、物件、声音或对白之后立即停笔，不再补“这意味着……”“已经没有退路”等局势说明。有限视角、对话潜台词、自然句式变化和反总结规则统一进入 writer Prompt。
- 生成后只做本地非阻断检查，记录实际字数，并提示长度越界、套话密集、总结式或解释式结尾；对白中的同类措辞不按旁白误报，也不自动追加 LLM 审校或重写调用。

默认规则：

- `autoReview` 在 C 端逐章默认计划中为 `false`。
- 确定性基础校验仍必须执行，空正文、明显截断、工程信息泄漏、异常格式和保存失败不能被当作正常完成。
- 下一章必需的事实与 canonical state 闭合仍属于进入下一章前置条件，不因关闭逐章深度审校而跳过；Timeline 保持展示和兼容边界，不重新成为默认热路径前置条件。
- `artifact_delta` 继续按幂等 checkpoint 执行；不需要阻塞下一章的资产允许在后台同步。
- 逐章确认是默认用户模式，用户确认当前版本后立即进入下一章准备和生成。
- 连续创作是可选模式，只允许运行到当前节奏段边界，并在断网、余额不足、用户暂停或程序退出时保存恢复点。
- 当前中转不能执行操作级消费上限，因此连续创作确认区只能展示当前余额与历史费用参考，不能承诺最高总价；每章仍是独立计费操作，到达剧情阶段边界必须停止，余额不足时不得启动下一章。
- 节奏段和整卷结束时暂停，由用户决定是否调用多章深度审校。
- 用户跳过阶段审校时记录可解释的 `reviewSkipped` 事实并继续，不得创建失败任务或阻塞消息。
- 专业模式仍可启用逐章 `autoReview`、局部修复和完整质量闭环，但不能改变 C 端默认值。

阶段审校消费该节奏段或整卷的稳定章节版本，输出整体结论、定位问题和推荐修复动作。它不得把每个内部 issue 拆成需要普通用户逐项调度的任务。

用户确认“这章可以”只提交当前版本作为下一章的生成基线，不代表锁定或发布；后续编辑该章时仍须通过版本历史、状态重算和必要的下游影响提示维护一致性。
...
- 章节唯一执行链定义如下：
  ...
  - 正文 writer、保存正文、资产同步和状态推进，以及执行计划启用时的接收闸门、patch repair、heavy repair 和复审，必须复用同一套 runtime 规则，不允许 route、旧 service 或导演分支各自再维护第二套正文执行实现。
...
- 生成后始终执行一次确定性正文自然度/退化检测。该检测只做本地文本规则检查，覆盖 AI 自述、占位符、工程词泄漏、截断、复读、破折号/省略号、否定翻转句、碎句和长段落等风险；它不调用 LLM，也不改变正文。
- 只有执行计划启用 `autoReview` 时，才追加结构化接收闸门，判断是否需要局部修文或人工确认。
...
- 启用逐章接收闸门时，该分支只等待 `acceptance`。Timeline 不属于 writer required context，也不构成下一章生成前置条件；未启用 Timeline 时不得生成“缺少 timeline context”的正文质量告警。
