...
叙事引擎的复杂度不能直接决定普通用户的默认界面。C 端默认主工作区已经确定为“带有智能作品目录的文本编辑器”：

- 左侧按正文、故事规划、角色、世界和更多资料组织作品内容。
- 中间统一阅读和编辑章节、大纲、卷规划、角色或世界文档。
- 右侧使用可收起的上下文 AI 助手。
- 自动导演、质量修复、资产回灌、任务运行和上下文装配在后台工作。

“导演台 + 世界模拟器 + 章节生产线”只作为未来专家模式、诊断模式或特定高密度资产视图的长期方向，不能成为普通用户进入作品后的默认首屏。

复杂状态需要先收敛成用户可以理解的创作进度、影响范围和唯一推荐动作。只有用户主动进入高级能力时，才展示世界状态快照、关键角色位置、势力动态、未兑现伏笔、质量债务、场景队列、推演、回滚、模型状态或任务队列。

具体 C 端布局和验收规则以[新手优先与整本小说完成原则](./beginner-first-novel-completion.md#单作品文档型创作台)为准。
