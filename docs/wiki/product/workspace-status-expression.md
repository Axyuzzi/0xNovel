...
- C 端原“创作消息”收敛为只读“创作进度”，只记录章节完成、暂停、中断、阶段完成和恢复历史，不承担必需操作。
- 需要用户确认的动作必须出现在对应章节、编辑器或阶段结束位置，并提供可执行的主按钮；不能只在消息列表里留下不可点击状态。
- C 端默认每章只显示“确认当前章并继续下一章”的主动作；深度审校由用户在剧情阶段或卷末决定是否启动。
...
- “第一章已完成”直接在正文底部显示“这章可以，继续写第二章”；创作进度只追加完成记录，不要求用户到进度抽屉处理。
...
- 必需操作只出现在“创作进度”但点击没有去处：把动作移回对应章节或阶段结束位置，进度列表只保留可定位的历史记录。
