...
### 2026-07-30（历史章节与连续创作进度）

- Windows 安装版现在会通过自有中转检查新版，发现更新后在产品顶部提示；下载过程会展示进度并在安装前校验文件完整性，安装前也会先保存正在编辑的正文。
- 必须更新的旧版本会保留本地阅读、编辑、保存、导出和备份，但会暂停新的联网创作操作，避免不兼容版本继续产生消费或错误数据。
- 修复长篇作品切换章节后左侧目录自动跳回顶部的问题。目录现在会按作品记住当前滚动位置，章节加载完成后仍停留在用户刚才浏览的位置。
- 修复回看第一章时错误显示“写完这一段，还剩 23 章”的问题。连续创作的起点和剩余章数现在只根据作品最新章节计算，不会跟随当前浏览章节倒退。
- 浏览历史章节时，右侧只说明作品当前已经写到第几章，并提供“回到最新章节继续”；不会再展示可能让人误以为需要重写后续章节的付费创作按钮。
- 当前剧情阶段已经写完时会明确显示“这一段已经写完”，等待阶段检查或下一段规划，不再给出含糊的继续入口。
- 书名和卷名会在现有规划调用内部先比较多个候选，优先保留能体现题材、独有冲突或关键意象的名称，并过滤“暗流涌动”“风云再起”等抽象万能标题。
- 第一章和后续章节统一使用具体事件标题，避免重复最近章节的核心词与句式；标题中的“第 N 章”、外围书名号和句末标点会在本地自动清理，目录编号仍由系统负责。
- 标题优化不增加模型调用，也不会产生额外扣费。
- 新书方向会明确主角可反复用于行动的核心优势、优势限制、持续阅读回报和阶段成长路线；这些字段适配悬疑、历史、言情和现实等题材，不会强制添加系统、超能力或打脸套路。
- 开篇规划会在前三章依次建立具体压力、展示优势及其边界、兑现首次回报或制造不可逆的新问题；第一章只读取全书核心主线、第一卷和当前阶段，避免被远期卷规划和结局干扰。
- 修复软件重启后点击“从当前进度继续”没有反应的问题。恢复操作现在会建立新的执行时间点，不会刚恢复就再次被判定为旧进程遗留任务并自动暂停。
- AI 修改与整章重写现在职责更清楚：修改只处理用户点名的问题，重写可以重新组织本章表达，但都不会提前泄露后续卷规划或强行追加固定爽点、奖励和悬念。
- 修改/重写只读取当前卷、当前阶段和最近章节尾段；异常代码围栏会自动清理，疑似截断的短结果不会进入候选稿。
- “从当前进度继续”点击后会立即显示“正在恢复连续创作”；如果服务仍无法恢复，会直接展示暂停原因，不再无反馈地回到原按钮。
- 阶段检查会聚焦当前阶段的章节证据和最近两章，不再把远期卷规划与大量完整正文一起发送给模型；检查结论更容易定位到具体章节，也能降低无关 Token 消耗。
- 后续剧情调整严格按“当前阶段、卷规划、全书骨架”逐级扩大范围；普通调整不会擅自改变全书前提或结局，跳过检查也不会生成虚构的质量问题。
- 下一阶段计划中的关键事件会写清人物行动、具体阻力和实际结果，减少“加强冲突”“危机逼近”等无法直接用于写章的抽象描述。

### 2026-07-29（中转账号、创作计费与连续写作）

- 注册现在会在后台自动完成登录并准备本产品专用的创作凭证，注册成功后可以直接进入产品，不需要理解或手动填写 API Key。
- 登录只使用产品账号和密码；界面不再提示邮箱登录，避免账号被错误转换后导致登录失败。
- 同一账号会优先复用仍有效的本产品凭证，不会误用账号下其他客户端的 Key；失效或过期凭证不会继续使用。
- “保持登录”仍只加密保存登录凭证、不保存密码，并会在重启后根据中转余额重新确认账号身份，避免作品打开到错误账号空间。
- 余额、消费记录、充值和 AI 创作统一使用当前登录账号的同一凭证；普通生成、流式正文和结构化规划均纳入发布前兼容检查。
- 充值订单的支付金额与到账积分不一致时会在展示付款码前停止，避免用户为异常订单付款。
- 正式桌面构建新增中转合同门禁；真实小额支付和真实模型扣费仍需在发布前使用生产账号完成一次留证验收。
- 修复测试安装包注册时提示“创作服务暂不可用”的问题；桌面包现在默认内置自有账号与模型中转地址，不会再因打包环境没有临时变量而生成空配置。
- 修复注册后首次生成提示“401 Invalid token”的问题；产品会创建真正可用于 AI 创作的专用凭证，旧的过期或零额度凭证不会继续使用。
- AI 服务异常不再展示 SDK 英文错误和调试链接；凭证失效时会直接提示重新登录，第一版由产品内部选择可用模型。
- 重新生成作品准备内容时会立即收起上一次失败提示，只保留当前生成状态；如果本次仍未完成，再展示本次结果，避免新旧状态同时出现。
- 我的账户可以正常展示缺少模型请求编号的充值与历史积分记录；待支付订单会降低查询频率、遇到限流自动放慢，并在二维码过期后停止查询。
- 创作模型改为按任务自动分工：规划分析侧重逻辑与成本，正文和改写侧重中文文笔，全书骨架与阶段审校使用独立模型把关；用户无需选择或理解模型。
- 下一段规划准备好后，创作台会先展示阶段名称、章节范围和目标，再让用户选择只写下一章或写完这一段，不会在规划边界给出含糊的继续按钮。
- “写完这一段”会逐章生成、保存和计费，实时展示当前章、阶段进度、累计积分与 Token；用户可以要求写完当前章后暂停，断网、退出或异常时也会保留恢复位置，并且不会跨过当前剧情阶段。
- 首章、后续章节、断点续写、AI 修改和整章重写现在共用正文表达规则：默认目标约 2600 字，开头更快进入人物与事件，结尾不再强行总结或升华，并减少说明文语气、整齐排比和重复套话。
- 章节任务会把“正文真正写出的章末画面”和“读者应该自行得出的判断”分开；模型在内部复查最后 300 字，避免用“这意味着”“已经没有退路”等旁白替读者解释局势。生成后的本地检查会提示解释式结尾，但不追加模型调用或额外扣费。
- 章节生成完成后会同时显示实际字数和建议范围；本地发现字数越界、套话密集或总结式结尾时只给出可选修改建议，不会偷偷再次调用模型或产生额外消费。

### 2026-07-28（本地 C 端创作主链与发布边界）

- 公开介绍站不再默认指向原项目 GitHub Pages，也不再请求第三方 GitHub 星标代理；打开当前站点不会因为遗留配置而访问原项目公开地址。
- 公开站只有在发布方显式配置自有站点地址后，才会生成 canonical、sitemap 和 robots 中的绝对站点入口；未配置时保持本地、相对和无外部指向的构建结果。
- 站点、正式桌面版和测试桌面版发布会在构建前检查原项目公开地址和非必要代理外联，避免旧地址重新进入发布物。
- 新桌面产品必须先登录或同步注册，保持登录只加密保存身份凭据而不保存密码；同一台电脑上的不同账号拥有彼此隔离的本地作品。
- 新用户可以从一句故事想法开始，依次确认故事方向、全书骨架、全部卷规划、当前剧情阶段和第一章；每次需要 AI 的付费生成都会停下来等待确认。
- 新创作台以作品目录、正文编辑器和明确下一步为核心；章节可直接修改，AI 修改先生成候选稿，采用前不会覆盖正文，关键节点可以恢复历史版本。
- 完成一章后可以确认并继续下一章；深度检查改到剧情阶段或卷末由用户决定，调整后续剧情时默认保护已经完成的章节。
- 账户页统一展示 0x积分、充值、消费、本地备份和恢复；用户也可以在二次确认后把当前账号的本机作品数据移入系统回收站。
- 断网时仍可阅读、编辑和保存本地作品；登录、充值和付费 AI 操作会在请求发出前停止，联网后可以从当前状态继续。
- 非创作页面在窄窗口下改为全宽内容和底部导航，不再被桌面侧栏挤压；帮助、隐私与更新说明使用普通用户语言。
- 正式桌面版只保留 C 端页面和自有中转出口，发布前检查签名、更新地址、外联边界、敏感日志和第三方依赖清单。
- 外联清理和产品改造不会改变原代码的版权或开源许可证义务；收费发布仍需先取得商业授权或完成律师确认的开源履约方案。
- 换账号时会先保存正在编辑的正文，再把后台正在进行的生成标记为“可能已产生消费、可重新续写”，然后退出并重新启动到登录页，避免切换中途的作品数据写到错误账号。
- 首页会根据你的状态自动变化：没有作品时直接显示一句故事想法输入；有作品时把最近一本放在最显眼位置并提供“继续写作”；作品准备还没完成的会引导你先完成准备再开始写正文。
- 新增“素材”入口，计划提供灵感示例和写作参考资料；当前版本是占位说明，可以先回到创作。

### 2026-07-17（0xNovelAgent 品牌、桌面端与模型配置）

- 产品界面、网页标题、公开文档、工作区包名和项目说明统一使用 `0xNovelAgent`，不同入口会展示一致的产品名称。
- Windows 桌面端的程序名、窗口标题、安装包、便携包、桌面快捷方式和开始菜单快捷方式统一使用 `0xNovelAgent`。
- Windows 可执行文件、启动页、浏览器标签和项目内图标已换成新的 `0x` 品牌素材，并保留深浅背景适配版本及可继续编辑的 SVG 母版。
- 桌面打包流程会正确暂存工作区依赖与 Electron 原生预编译模块；打包后的本地服务路径和启动链路已修复，安装版与便携版可以完成结构校验和真实启动烟测。
- 自定义模型供应商会持久保存已获取或刷新的完整模型列表；重新进入模型路由页面后，仍可为同一供应商选择多个模型，不需要为每个模型重复创建供应商。
- 项目入口和手动更新入口已切换到自有 Gitee 仓库；桌面自动更新只接受打包时显式配置的自有通用更新地址，未配置时保持关闭，不再访问原 GitHub 更新源。
