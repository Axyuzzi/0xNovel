...
本仓库的长期业务逻辑主要在 [`server/tests/`](../../server/tests/) 下，使用 **Node 内置 `node:test`** 与 **`node:assert/strict`**。默认测试入口会先构建 `@0xnovelagent/shared` 与 `@0xnovelagent/server`，再运行日常快速测试。
...
# 已经构建过时，只运行后端快速测试文件
pnpm --filter @0xnovelagent/server test:node

# 运行真实 Prisma / 迁移 / 兼容性等重型集成测试
pnpm --filter @0xnovelagent/server test:integration
