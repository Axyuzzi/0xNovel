...
function buildManagedServerCommand(): {
  ...
  return {
    ...
    args: ["--filter", "@0xnovelagent/server", "start"],
    cwd: resolveWorkspaceRoot(),
  };
}
...
function startPackagedManagedServer(
  ...
  const child = utilityProcess.fork(resolvePackagedServerEntry(), [], {
    ...
    env: {
      ...
      OXNOVEL_RELAY_PLANNER_MODEL: releasePolicy.plannerModel,
      OXNOVEL_RELAY_WRITER_MODEL: releasePolicy.writerModel,
      OXNOVEL_RELAY_REVIEW_MODEL: releasePolicy.reviewModel,
