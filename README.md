...
### 2026-07-28（本地 C 端创作主链与发布边界）

- 新桌面产品使用强制登录、同步注册和账号隔离的本地作品空间，不保存用户密码。
- 新用户可以从一句想法开始，分五步确认作品准备，并在简化创作台中逐章写作、修改、采用候选稿和恢复版本。
- 阶段/卷末检查、后续剧情调整、0x积分、充值、消费、备份恢复和本机数据删除已进入统一 C 端流程。
- 断网不影响本地阅读与编辑，但会阻止登录、充值和付费 AI 请求；窄窗口使用全宽内容和底部导航。
- 换账号会先保存正文、标记在途生成、重启到登录页，避免切换中途写到错误账号；首页根据是否有作品自适应；新增“素材”入口。
- 正式构建只保留 C 端页面和自有中转出口，并在发布前检查签名、更新地址、敏感日志、外联边界和第三方依赖清单。
- 收费发布仍需完成真实中转/支付、商业授权与法务、正式签名和全新 Windows 前三章验收。
...
pnpm db:seed
pnpm db:studio
pnpm --filter @0xnovelagent/server test
pnpm --filter @0xnovelagent/server test:routes
pnpm --filter @0xnovelagent/server test:book-analysis
```
