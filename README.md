...
### 2026-07-29（中转账号、登录保持与充值合同）

- 注册成功后会自动登录并准备本产品专用的创作凭证，用户不需要理解或填写 API Key。
- 登录入口只接受产品账号和密码；保持登录仍只加密保存凭证、不保存密码，重启后会重新确认账号身份。
- 同一账号只复用仍有效的本产品凭证，余额、消费、充值和 AI 创作统一使用当前账号。
- 充值金额与到账积分不一致时会在付款前停止；普通生成、流式正文和结构化规划都进入正式构建检查。
- 真实小额支付和真实模型扣费仍需在生产账号上完成一次发布前留证验收。
- 修复安装包因空中转配置导致注册直接提示“创作服务暂不可用”的问题，后续 Beta 和正式包都会内置自有中转地址。
- 修复注册后首次生成提示“401 Invalid token”的问题；专用创作凭证会使用正确的有效期与额度规则，底层英文调试信息也不会再直接展示给用户。
