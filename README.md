...
- Windows 安装版会通过自有中转提示、下载和校验新版本，并在启动安装程序前保存当前正文。
- 长篇作品切换章节后，左侧目录会保持当前滚动位置，不再每次跳回顶部。
...
- 重新生成作品准备内容时会收起上一次失败提示，只显示当前生成状态，避免新旧结果同时出现。
- 我的账户可以展示缺少模型请求编号的历史积分记录；待支付订单会降低查询频率，并在二维码过期后停止查询。
- AI 会按任务自动选择适合的中转模型：规划分析、正文改写和关键审校分别处理，兼顾长篇逻辑、中文文笔与生成成本，用户不需要选择模型。
- 下一段规划准备好后会先展示阶段范围和目标，用户可以只写下一章，也可以选择“写完这一段”。
- 回看历史章节不会改变真实创作进度；右侧会提示当前最新章节并提供返回入口，连续创作的剩余章数始终按作品最新章节计算。
- 连续写作逐章保存和计费，显示阶段进度、累计积分与 Token；可以在当前章结束后暂停，异常或重启后也会保留恢复位置，并且不会跨过当前剧情阶段。
- 首章、后续章节、断点续写和 AI 改写统一使用约 2600 字的正文合同，开头直接进入人物与事件，章末避免总结升华，并减少说明文语气、整齐排比和重复套话。
- 章节任务会分开“实际写出的章末画面”和“留给读者的推断”，正文生成时复查最后 300 字，避免在有效动作后再补一句局势解释。
- 生成完成后会显示本章实际字数和建议范围；本地表达检查只给修改建议，不会偷偷追加模型调用或产生额外消费。
- 书名、卷名和章节标题会从具体题材、冲突、物件和事件中筛选，避免抽象万能标题与相邻章节重复；这项优化复用现有调用，不增加消费。
- 新书方向会明确题材自适应的破局优势、限制代价、持续回报和成长路线；开篇前三章更快建立压力、优势应用和首次兑现，首章不会读取无关的远期卷规划。
- 修复重启后连续创作点击“从当前进度继续”没有反应的问题，恢复后会真正从已保存章节继续。
- AI 修改与整章重写升级为 v4：只读取当前卷、当前阶段和最近章节尾段，精修改动更克制，整章重写不提前泄露后续规划；短结果与外围代码围栏会被本地拦截或清理。
- 连续创作恢复按钮会立即显示恢复状态，服务仍处于暂停时会明确展示原因。
- 阶段检查与后续调整升级为 v2：局部检查只读取当前阶段证据、最近两章和相邻卷，远期卷不会分散模型注意力；调整范围严格按当前阶段、卷规划、全书骨架逐级扩大。
- 下一阶段规划会把每个关键事件写成具体行动、阻力和结果，跳过检查不会虚构质量问题或擅自改变上层规划。
