...
```bash
pnpm --filter @0xnovelagent/site dev
```
...
```bash
pnpm --filter @0xnovelagent/site build
```
...
`.github/workflows/site-pages.yml` 会在推送到 `main` 或手动触发时构建 `@0xnovelagent/site`，并把 `site/dist` 发布到 GitHub Pages。
...
5. 运行 `pnpm --filter @0xnovelagent/site build`。
