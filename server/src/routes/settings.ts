...
import type { BuiltinLLMProvider, LLMProvider } from "@0xnovelagent/shared/types/llm";
import { z } from "zod";
...
import {
  getProviderModelCatalogMap,
  saveProviderModelCatalog,
} from "../services/settings/ProviderModelCatalogService";
...
function buildCustomProviderStatus(item: {
  ...
  concurrencyLimit?: number | null;
  requestIntervalMs?: number | null;
}, imageModel: string | undefined, savedModels: string[]): CustomProviderStatus {
  const currentModel = normalizeOptionalText(item.model) ?? "";
  const currentBaseURL = normalizeOptionalText(item.baseURL) ?? "";
  const models = Array.from(new Set([currentModel, ...savedModels].filter(Boolean)));
  return {
...
router.get("/api-keys", async (_req, res, next) => {
  try {
    ...
    const customProviderIds = keys
      .filter((item) => !isBuiltInProvider(item.provider))
      .map((item) => item.provider);
    const modelCatalogMap = await getProviderModelCatalogMap(customProviderIds);
    ...
    const customProviders = keys
      ...
      .map((item) => buildCustomProviderStatus(
        ...
        modelCatalogMap.get(item.provider) ?? [],
      ));
    ...
    try {
      ...
      try {
        ...
        if (!isBuiltInProvider(provider)) {
          await saveProviderModelCatalog(provider, models);
        }
      } catch {
