...
test("GET /api/settings/api-keys exposes custom OpenAI-compatible providers", async () => {
  ...
  prisma.appSetting.findMany = async () => ([
    {
      key: "provider.modelCatalog.custom_storyhub",
      value: JSON.stringify(["story-model", "story-model-pro", "summary-model"]),
    },
  ]);
  ...
  try {
    ...
    assert.deepEqual(custom.models, ["story-model", "story-model-pro", "summary-model"]);
  } finally {
    prisma.aPIKey.findMany = originalFindMany;
...
test("POST /api/settings/custom-providers creates a custom provider entry", async () => {
  ...
  prisma.appSetting.upsert = async ({ where, create, update }) => {
    savedModelCatalog = { where, create, update };
    return {
      key: create.key,
      value: create.value,
      createdAt: new Date(),
  ...
  try {
    ...
    assert.deepEqual(savedModelCatalog, {
      where: { key: "provider.modelCatalog.custom_storyhub_gateway" },
      create: {
        key: "provider.modelCatalog.custom_storyhub_gateway",
        value: JSON.stringify(["story-model", "story-model-pro"]),
      },
      update: { value: JSON.stringify(["story-model", "story-model-pro"]) },
    });
  } finally {
    prisma.aPIKey.findUnique = originalFindUnique;
